...
async def example_usage():
    ...
    try:
        ...
        # 示例 2：批量解压缩
        compressed_data_list = [b'...'] * 10  # 假设有多个压缩数据
...
#### 2.5.3 配置文件示例 (databases/blacklist.json)
...
**情况 1：动态黑名单（基于访问频率）**
```python
class RateLimitBlacklist(BlacklistManager):
    """带访问频率限制的黑名单管理器"""
    
    def __init__(self, config_file='databases/blacklist.json'):
        super().__init__(config_file)
        
        # 访问频率记录：{ip: [timestamp1, timestamp2, ...]}
    ...
    async def check_rate_limit(self, client_ip: str) -> tuple:
        ...
        # 清理过期记录
        if client_ip in self._access_records:
            self._access_records[client_ip] = [
                t for t in self._access_records[client_ip]
                if current_time - t < self.rate_limit['time_window']
            ]
        
        # 检查频率
        if client_ip not in self._access_records:
            self._access_records[client_ip] = []
        
        request_count = len(self._access_records[client_ip])
        
        if request_count >= self.rate_limit['max_requests']:
            # 超过限制，添加到临时黑名单
            await self.add_to_blacklist(client_ip, 'ip')
            
            # 计算重试时间
            oldest_request = min(self._access_records[client_ip])
            retry_after = int(oldest_request + self.rate_limit['time_window'] - current_time)
...
**情况 2：黑名单缓存优化**
```python
class CachedBlacklistManager(BlacklistManager):
    """带缓存的黑名单管理器"""
    
    def __init__(self, config_file='databases/blacklist.json'):
        super().__init__(config_file)
        
        # 缓存：{(ip, url, domain): (is_blocked, reason, timestamp)}
        self._cache: Dict[tuple, tuple] = {}
        
        # 缓存过期时间
        self.cache_ttl = 60  # 60秒
    
    async def is_blocked(self, client_ip: str, url: str, domain: str) -> tuple:
        """带缓存的黑名单检查"""
        cache_key = (client_ip, url, domain)
        current_time = time.time()
        
        # 检查缓存
        if cache_key in self._cache:
            is_blocked, reason, timestamp = self._cache[cache_key]
            
            if current_time - timestamp < self.cache_ttl:
                return is_blocked, reason
        
        # 执行检查
        is_blocked, reason = await super().is_blocked(client_ip, url, domain)
        
        # 更新缓存
        self._cache[cache_key] = (is_blocked, reason, current_time)
...
class LoggedBlacklistManager(BlacklistManager):
    """带日志记录的黑名单管理器"""
    ...
    async def is_blocked(self, client_ip: str, url: str, domain: str) -> tuple:
        ...
        if is_blocked:
            logger.warning(
                f"Blacklist blocked | IP: {client_ip} | URL: {url} | Domain: {domain} | Reason: {reason}"
            )
        else:
            logger.debug(
                f"Blacklist allowed | IP: {client_ip} | URL: {url} | Domain: {domain}"
            )
...
#### 2.6.3 配置文件示例 (databases/scripts.json)
...
(function() {
    ...
    function highlightTarget() {
        ...
        links.forEach(link => {
            // 添加高亮样式
            link.style.outline = '2px solid #FF5722';
            link.style.outlineOffset = '2px';
            
            // 添加鼠标悬停提示
            link.title = 'SilkRoad Proxy: ' + link.href;
        });
        
        console.log('SilkRoad: Highlighted ' + links.length + ' links for target: ' + target);
    }
...
async def example_usage():
    # 创建脚本注入管理器
    script_injector = ScriptInjector('databases/scripts.json')
    ...
    try:
        ...
        injected_html = await script_injector.inject_scripts(html, url)
        
        print("Injected HTML:")
        print(injected_html)
        
        # 添加新脚本
        await script_injector.add_script(
            name='custom-script',
            file='custom.js',
            enabled=True,
            position='body_end',
        ...
        # 移除脚本
        await script_injector.remove_script('custom-script')
        
        # 热重载配置
        await script_injector.reload_config()
        
        # 获取统计信息
        stats = script_injector.get_stats()
        print(f"Script injection stats: {stats}")
...
**情况 1：脚本加载失败**
```python
class RobustScriptInjector(ScriptInjector):
    """健壮的脚本注入管理器"""
...
class ScriptInjector:
    ...
    async def inject_scripts(self, html: str, url: str, content_type: str = 'text/html') -> str:
        ...
        for script_name in self._scripts.keys():
            marker = f'<!-- SilkRoad Script: {script_name} -->'
            if marker in html:
                existing_scripts.add(script_name)
        
        # 获取适用的脚本（排除已注入的）
        scripts = await self.get_scripts_for_url(url, content_type)
        scripts = [s for s in scripts if s['name'] not in existing_scripts]
        ...
        for script in scripts:
            script['content'] = (
                f"<!-- SilkRoad Script: {script['name']} -->\n"
                f"{script['content']}\n"
                f"<!-- End SilkRoad Script: {script['name']} -->"
            )
...
class ScriptInjector:
    ...
    async def _inject_to_body(self, html: str, scripts: List[dict]) -> str:
        """注入脚本到 body（保证执行顺序）"""
        # 按优先级排序
        scripts.sort(key=lambda x: x['config'].get('priority', 0), reverse=True)
        ...
        for i, script in enumerate(scripts):
            ...
            # 添加顺序标记
            tag = (
                f'<script type="text/javascript" data-silkroad-order="{i}">\n'
                f'{content}\n'
                f'</script>'
            )
            script_tags.append(tag)
...
class SilkRoadProxyV2:
    ...
    def __init__(self):
        ...
        self.thread_pool = ThreadPoolManager(max_workers=8)
        ...
        self.cache_manager = CacheManager(
            ...
            max_disk_cache_size=1024 * 1024 * 1024,
            default_ttl=3600
        )
        
        self.blacklist_manager = BlacklistManager('databases/blacklist.json')
        
        self.script_injector = ScriptInjector('databases/scripts.json')
    ...
    async def start(self):
        ...
        print("SilkRoad V2 Proxy Server started")
    ...
    async def handle_request(self, client_ip: str, url: str, method: str = 'GET'):
        """处理请求（集成所有模块）"""
        # 1. 黑名单检查
        from urllib.parse import urlparse
        parsed = urlparse(url)
        ...
        is_blocked, reason = await self.blacklist_manager.is_blocked(client_ip, url, domain)
        
        if is_blocked:
            return {
                'status': 403,
                'body': f'Blocked: {reason}'
            }
        ...
        # 3. 缓存检查
        cached_data = await self.cache_manager.get(url, method)
        ...
        # 4. 使用连接池获取数据
        connection = await self.connection_pool.get_connection(domain, 443, True)
...
```json
{
  ...
  "cache": {
    "maxMemoryCacheSize": 104857600,
    "maxDiskCacheSize": 1073741824,
    "defaultTTL": 3600,
    "diskCacheDir": "./cache"
  },
  "blacklist": {
